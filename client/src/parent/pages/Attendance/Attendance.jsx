...
function statusStyle(status) {
  switch (status) {
    case "present":
      return { bg: "rgba(187,247,208,0.85)", color: "#166534" };
    case "absent":
      return { bg: "rgba(254,202,202,0.85)", color: "#991b1b" };
    case "holiday":
      return { bg: "rgba(189,221,252,0.60)", color: "#384959" };
    case "upcoming":
      return { bg: "rgba(241,245,249,0.80)", color: "#94a3b8" };
    default:
      return { bg: "rgba(241,245,249,0.80)", color: "#94a3b8" };
  }
}
...
export default function ParentAttendance() {
      ...
      try {
        ...
        const list = raw.map((s) => ({
          ...
          name: s.personalInfo
            ...
            : s.name,
          className:
            s.enrollments?.[0]?.classSection?.name ??
  ...
  const handleMonthChange = (e) => {
    const sel = data?.availableMonths?.find(
            ...
            {!selectedChild && (
              ...
              >
                ...
                <p style={{ color: C.mid, fontSize: 13, margin: 0 }}>
                  Select a child above to view their attendance.
                </p>
              </div>
            )}
                      ...
                      <>
                                ...
                                const attendanceDetail =
                                  ...
                                  item.status === "absent"
                                    ? [
                                        item.source === "manual"
                                          ? `Marked by ${
                                              item.markedByName || "Teacher"
                                            }`
                                          : item.source === "biometric"
                                          ? "Biometric punch"
                                          : null,
                                        item.hasPunch
                                          ? `In ${item.firstPunchFmt}${
                                              item.lastPunchFmt
                                                ? ` · Out ${item.lastPunchFmt}`
                                                : ""
                                            }`
                                          : null,
                                      ]
                        ...
                        >
                          {[
                            ...
                            {
                              bg: "rgba(254,202,202,0.85)",
                              color: "#991b1b",
                              label: "Absent",
                            },
                            {
                              bg: "rgba(189,221,252,0.60)",
                              color: "#384959",
                              label: "Holiday",
                            },
                          ].map(({ bg, color, label }) => (
                  ...
                  <div className="att-card" style={{ padding: "20px 18px" }}>
                    ...
                    >
                      ...
                      >
                        ...
                        <div
                          style={{ fontSize: 11, color: C.mid, marginTop: 3 }}
                        >
                          {pct >= 95
                            ? "On Track ✓"
                            : `${(95 - pct).toFixed(1)}% to go`}
                        </div>
                      </div>
                      ...
                      {stats.absent > 0 && (
                        ...
                        >
                          ...
                          >
                            {stats.absent} absent day
                            {stats.absent > 1 ? "s" : ""} this month
                          </div>
                        </div>
                      )}
                    </div>
