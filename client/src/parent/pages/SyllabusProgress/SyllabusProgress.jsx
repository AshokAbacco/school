...
function StatCards({ data }) {
  ...
  const completedSubjects =
    data.subjects?.filter((s) => s.percentage === 100).length ?? 0;
  ...
  const cards = [
    ...
    },
    {
      ...
      value: `${overall.pct ?? 0}%`,
      sub: `${overall.completed ?? 0} chapters`,
      accent: accentColor,
      Icon: BarChart2,
    },
  ];
...
function SubjectCard({ subject, isOpen, onToggle }) {
  ...
  return (
    <div
      onClick={onToggle}
      className={`subj-card ${isOpen ? "active" : ""}`}
      style={{
        border: isOpen
          ? `1.5px solid ${C.sky}`
          : `1.5px solid ${C.borderLight}`,
    ...
    >
      ...
      >
        ...
        <span style={{ fontSize: "1rem", fontWeight: 800, color: barColor }}>
          {percentage}%
        </span>
      </div>
  ...
  const fetchChildren = async () => {
    try {
      ...
      const list = (json.data || []).map((s) => ({
        studentId: s.id,
        name: s.name,
        className: s.enrollment?.className ?? null,
      }));
      setChildren(list);
  ...
  const fetchProgress = async () => {
    try {
      ...
      const res = await fetch(
        `${API}/api/parent/syllabus-progress?studentId=${selectedChild}`,
        { headers: { Authorization: `Bearer ${getToken()}` } },
      );
      const json = await res.json();
      setData(json);
          ...
          <div className="subject-grid">
            {data.subjects?.map((sub) => (
              <SubjectCard
                ...
                subject={sub}
                isOpen={false}
                onToggle={() => setOpenCardId(sub.subjectId)}
              />
            ))}
          </div>
        </div>
