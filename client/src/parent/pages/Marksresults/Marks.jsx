...
function NotPublished({ isMobile }) {
  return (
    ...
    >
      ...
      >
        Marks have not been released for this exam yet. Check back once the
...
export default function ParentMarks() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const list = (
          Array.isArray(data) ? data : data.students ?? data ?? []
        ).map((s) => ({
          studentId: s.id,
          name: s.personalInfo
    ...
    (async () => {
      try {
        const data = await apiFetch(
          `/api/parent/marks/exam-groups?studentId=${selectedChild}`,
        );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await apiFetch(
          `/api/parent/marks/report/${selectedId}?studentId=${selectedChild}`,
        );
        setReportData(data);
      } catch (e) {
        if (
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      await downloadReportPDF(enriched);
    } finally {
            ...
            {!selectedChild && (
              ...
              >
                ...
                <p style={{ color: C.mid, fontSize: 13, margin: 0 }}>
                  Select a child above to view their marks and report card.
                </p>
              </div>
            )}
            ...
            {selectedChild &&
              ...
              !errorGroups && (
                ...
                >
                  ...
                  <p style={{ color: C.mid, fontSize: 13, margin: 0 }}>
                    No published exam results for{" "}
                    {activeChild?.name ?? "this student"}.
                  </p>
                </div>
              )}

            {!loadingReport && notPublished && (
              <NotPublished isMobile={isMobile} />
            )}

            {(loadingReport || showReport) && (
              <div
                style={{ animation: showReport ? "fadeUp 0.4s ease" : "none" }}
              >
                <SummaryCards
                ...
                <div
                  style={{
                    display: "grid",
                    gridTemplateColumns:
                      isMobile || isTablet ? "1fr" : "1fr 284px",
                    gap: isMobile ? 12 : 18,
                    alignItems: "start",
                  }}
                >
