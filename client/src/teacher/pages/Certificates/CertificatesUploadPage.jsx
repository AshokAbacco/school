    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
        ...
        {cert.uploadedBy?.name && (
          <MetaRow label={`👤 Uploaded by ${cert.uploadedBy.name}`} />
        )}
      </div>
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch(
          `${API_BASE}/api/teacher/certificates/students?search=${encodeURIComponent(
            studentSearch,
          )}`,
              ...
              >
                ...
                {file ? (
                  ...
                  <>
                    ...
                    >
                      Drop file here or{" "}
                      <span style={{ color: C.sky }}>browse</span>
                    </p>
                    <p
                      style={{
                        fontSize: 11,
  ...
  return (
    <Modal title="Delete Certificate?" onClose={onClose}>
      ...
      >
        ...
        <strong style={{ color: C.text }}>{cert?.title}</strong> for{" "}
        <strong style={{ color: C.text }}>{cert?.studentName}</strong>. This
        cannot be undone.
      </p>
      ...
      >
        ...
        >
          <div>
            ...
            >
              {cert.studentName} · {cert.academicYear}
            </p>
          </div>
          <button
            onClick={onClose}
...
export default function CertificatesUploadPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/teacher/certificates/${deletingCert.id}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
