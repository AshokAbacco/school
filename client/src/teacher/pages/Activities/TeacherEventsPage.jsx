  ...
  return (
    ...
    >
      ...
      {["", ...classes.map((c) => c.id)].map((id) => {
        ...
        const active =
          (!id && !selectedClassId) || (id && selectedClassId === id);
        return (
          <button
            key={id || "all"}
            onClick={() => onChange(id === selectedClassId ? "" : id)}
            style={{
              padding: "4px 10px",
              borderRadius: 20,
              fontSize: 11,
...
function MemberPicker({
  ...
  const filtered = students.filter(
    (s) =>
      ...
      (!classId || s.classSection?.id === classId) &&
  ...
  const removeMember = async (studentId) => {
    try {
      await apiFetch(
        `${API_EVENTS}/${eventId}/teams/${team.id}/members/${studentId}`,
        { method: "DELETE" },
      );
      onMembersChanged();
    } catch (e) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      >
        ...
        {classId && (
          ...
          >
            <Filter size={11} /> Showing <strong>{selectedClassName}</strong>{" "}
            only —
            <button
              onClick={() => setClassId("")}
              style={{
          ...
          >
            {filtered.map((s) => (
              <div
                ...
                onMouseEnter={(e) =>
                  (e.currentTarget.style.background = `${C.sky}10`)
                }
                onMouseLeave={(e) =>
        ...
        {(q || classId) && filtered.length === 0 && (
          ...
          >
            {classId && !q
              ? `All students from ${selectedClassName} are already in this group.`
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {m.student?.classSection && (
                  ...
                  >
                    {m.student.classSection.name}
                  </p>
                )}
              </div>
              <button
                onClick={() => removeMember(m.student?.id ?? m.studentId)}
                style={{
                  background: "none",
                  border: "none",
                  cursor: "pointer",
...
function GroupManager({ event, pushToast }) {
  ...
  const loadGroups = useCallback(
    async (silent = false) => {
      ...
      try {
        ...
        setGroups(r.data);
        setSelGroup((prev) =>
          prev ? r.data.find((g) => g.id === prev.id) ?? prev : null,
        );
      } catch (e) {
        pushToast(e.message, "error");
      } finally {
  ...
  const loadStudents = useCallback(async () => {
    try {
      const eligibleClassIds =
        event.eligibleClasses
          ?.map((ec) => ec.classSection?.id)
          .filter(Boolean) ?? [];
      if (eligibleClassIds.length > 0) {
        const results = await Promise.all(
          eligibleClassIds.map((csId) =>
            apiFetch(`${API_STUDENTS}?classSectionId=${csId}`),
          ),
        );
        const seen = new Set();
        setStudents(
  ...
  const loadClasses = useCallback(async () => {
    try {
      const eligible =
        event.eligibleClasses?.map((ec) => ec.classSection).filter(Boolean) ??
        [];
      if (eligible.length > 0) {
        setClasses(eligible);
      } else {
        const r = await apiFetch(API_CLASSES);
        setClasses(r.data ?? []);
      }
    } catch (_) {}
  }, [event.id]);
  ...
  useEffect(() => {
    ...
    loadStudents();
    loadClasses();
  }, [loadGroups, loadStudents, loadClasses]);

  const createGroup = async () => {
    if (!newName.trim()) return;
    setSaving(true);
    try {
      await apiFetch(`${API_EVENTS}/${event.id}/teams`, {
        method: "POST",
  ...
  const deleteGroup = async (id) => {
    ...
    try {
      await apiFetch(`${API_EVENTS}/${event.id}/teams/${id}`, {
        method: "DELETE",
      });
            ...
            {creating && (
              ...
              >
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <PrimaryBtn small onClick={createGroup} loading={saving}>
                    <Plus size={11} /> Create
                  </PrimaryBtn>
                  <OutlineBtn
                    small
              ...
              groups.map((g, gi) => (
                <div
                  ...
                  style={{
                    ...
                    cursor: "pointer",
                    border: `1.5px solid ${
                      selGroup?.id === g.id ? C.sky : C.borderLight
                    }`,
                    background: selGroup?.id === g.id ? `${C.sky}10` : C.white,
                  }}
                  onMouseEnter={(e) => {
                    if (selGroup?.id !== g.id)
      ...
      <div>
        {selGroup ? (
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  ...
                  >
                    {hasEligibleClasses
                      ? "Students filtered to eligible classes"
                      : "Pick a class to filter, or search all students"}
                  </p>
                </div>
              </div>
            </div>
          ...
          >
            ...
            >
              Click any group on the left to add students
            </p>
          </div>
        )}
      </div>
...
export default function TeacherEventsPage() {
  ...
  if (selEvent)
    return <EventDetail event={selEvent} onBack={() => setSelEvent(null)} />;
          ...
          <div style={{ padding: 18 }}>
            {loading ? (
              ...
              >
                ...
                >
                  Admin creates events — they will appear here once created.
                </p>
              </div>
            ) : (
              <div
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <strong style={{ color: C.text }}>{partCount}</strong>{" "}
                          participant{partCount !== 1 ? "s" : ""}
                        </span>
                      </div>
                    </div>
                  );
