...
function buildTxnRows(txn, allCategoryRows) {
  ...
  if (allCategoryRows && allCategoryRows.length > 0) {
    return allCategoryRows.map((cat) => {
      ...
      const cumulativePaid = item ? Number(item.cumulativePaid ?? paid) : null; // unknown for categories not in this txn
  ...
  return txn.items.map((item) => {
    ...
    return {
      id: String(
        item.studentFeeCategoryId || item.categoryName || Math.random(),
      ),
...
export function InvoiceModal({
  ...
  // Fallback prefix if the school hasn't picked one yet in Finance settings
    ...
    (async () => {
      try {
        ...
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/api/school/logo`,
          {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (Array.isArray(data) && data.length > 0) {
            setAllCategoryRows(
              buildCategoryRows({ ...student, feeCategories: data }),
            );
          } else {
    ...
    const fetchHistory = async () => {
      ...
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          console.log(
            "[FeesInvoice] Payment History API returned:",
            JSON.stringify(data),
          );
          ...
          if (data.length > 0) {
            ...
            console.log(
              "[FeesInvoice] ✅ Dropdown:",
              data.length,
              "entries. Auto-selected:",
              data[0].label,
            );
          } else {
            console.warn(
              "[FeesInvoice] ⚠️  Empty — dropdown will NOT appear. Student paidAmount may be 0.",
            );
          }
        } else {
        ...
        console.error(
          "[FeesInvoice] ❌ paymentHistory fetch threw:",
          e.message,
        );
      } finally {
  ...
  const selectedTxn =
    paymentHistory.find((t) => t.id === selectedTxnId) || null;

  // Real invoice number is generated server-side per transaction
  // (ABC-INV-00001, see recordSimplePayment). Older/legacy transactions that
  // predate this feature don't have one — fall back to a school-prefixed,
  // student-based number so the invoice never shows a bare/unprefixed number.
  const invoiceNo =
    selectedTxn?.invoiceNumber ||
    `${fallbackPrefix}-INV-${String(student.id || "").padStart(5, "0")}`;

  // rows for the selected transaction, or fallback to all-time rows
  const baseRows =
    selectedTxn &&
    Array.isArray(selectedTxn.items) &&
  ...
  const handleDownload = async () => {
    ...
    doc.text(
      ...
      textX,
      logoData ? logoY + 18 : 24,
    );
    if (schoolAddress) {
      doc.setFontSize(8);
      doc.setTextColor(140, 175, 200);
      doc.text(schoolAddress, textX, logoData ? logoY + 25 : 32);
    }
    ...
    doc.text(
      `Status: ${grandPending === 0 ? "PAID" : "PARTIALLY PAID"}`,
      W - m,
      headerH + 7,
      { align: "right" },
    );
    ...
    doc.roundedRect(m, y - 6, W - m * 2, boxH, 3, 3, "F");
    doc.setFont("helvetica", "bold");
    doc.setFontSize(9);
    doc.setTextColor(28, 48, 68);
    ...
    if (student.address) {
      ...
      doc.setFont("helvetica", "normal");
      doc.text(
        doc.splitTextToSize(student.address, W - m * 2 - 34),
        m + 22,
        y + 30,
      );
    }
    ...
    visibleRows.forEach((row, i) => {
      // or rows.forEach(...) in Studentfinance.jsx
    ...
    const bx = W - m - 80;
    doc.setFillColor(240, 247, 252);
    doc.roundedRect(bx, y, 80, 46, 3, 3, "F");
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    ...
    doc.line(bx + 4, y + 24, bx + 76, y + 24);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(10);
    doc.setTextColor(28, 48, 68);
    doc.text("Balance Due:", bx + 4, y + 34);
    doc.setTextColor(
      grandPending === 0 ? 28 : 180,
      grandPending === 0 ? 90 : 30,
      grandPending === 0 ? 50 : 30,
    );
    ...
    doc.text(
      schoolName
        ? `${schoolName} · System-generated receipt. No signature required.`
    ...
    doc.save(
      `Receipt_${(student.name || "Student").replace(/\s+/g, "_")}_${invoiceNo}.pdf`,
    );
  };
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="text-white font-bold text-sm truncate">
                {schoolName || "Student Invoice"}
              </div>
              <div className="text-blue-200 text-xs truncate">
                {invoiceNo} · {receiptDateLabel}
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto min-h-0 px-5 py-4 space-y-4">
          ...
          {schoolName && (
            <div className="flex items-center gap-3 bg-gradient-to-r from-[#f0f7fc] to-[#e4f0f8] border border-[#c8dff0] rounded-xl px-4 py-3">
              ...
              <div>
                <span className="block text-[10px] font-bold text-[#4A6B80] uppercase tracking-wider">
                  Issued By
                </span>
                <span className="block text-sm font-bold text-[#1C3044]">
                  {schoolName}
                </span>
                {schoolAddress && (
                  <span className="block text-xs text-[#4A6B80]">
                    {schoolAddress}
                  </span>
                )}
              </div>
          ...
          <div className="border border-[#e0eef6] rounded-xl p-4">
            <div className="text-[10px] font-bold text-[#4A6B80] uppercase tracking-wider mb-3">
              Student Details
            </div>
            <div className="grid grid-cols-2 gap-x-6 gap-y-3">
              <div>
                <div className="text-[11px] text-[#4A6B80]">Name</div>
                <div className="text-sm font-semibold text-[#1C3044]">
                  {student.name}
                </div>
              </div>
              <div>
                <div className="text-[11px] text-[#4A6B80]">Email</div>
                <div className="text-sm font-semibold text-[#1C3044] break-all">
                  {student.email}
                </div>
              </div>
              <div>
            ...
            {student.address && (
              <div className="mt-3 pt-3 border-t border-[#e0eef6]">
                ...
                <div className="text-sm font-semibold text-[#1C3044] mt-0.5 leading-relaxed">
                  {student.address}
                </div>
              </div>
            )}
          ...
          <div className="border border-[#e0eef6] rounded-xl overflow-hidden">
            ...
            <div className="px-4 py-3 border-b border-[#e0eef6] flex items-center justify-between gap-3 flex-wrap">
              ...
              {!historyLoading && paymentHistory.length > 0 && (
                <div className="relative flex-shrink-0" ref={dropdownRef}>
                  ...
                  >
                    <span className="text-[10px] font-normal text-[#4A6B80] mr-0.5">
                      Payment Date
                    </span>
              ...
              {historyLoading && (
                <span className="text-[10px] text-[#4A6B80] animate-pulse">
                  Loading payments…
                </span>
              )}
            </div>
            ...
            {loading ? (
              <div className="text-center py-8 text-sm text-[#4A6B80]">
                Loading…
              </div>
            ) : (
                ...
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr className="bg-gradient-to-r from-[#1C3044] to-[#27435B]">
                      {["Fee Category", "Total", "Paid", "Pending", ""].map(
                        (h, idx) => (
                          <th
                            key={idx}
                  ...
                  <tbody>
                    {baseRows.length === 0 ? (
                      <tr>
                        <td
                          colSpan={5}
                          className="px-4 py-6 text-center text-sm text-[#4A6B80]"
                        >
                      ...
                      baseRows.map((row, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td
                              className={`px-4 py-2.5 font-bold whitespace-nowrap ${isHidden ? "text-[#bbb]" : "text-[#27435B]"}`}
                            >
                              ₹{fmt(row.total)}
                            </td>
                            <td
                              className={`px-4 py-2.5 font-semibold whitespace-nowrap ${isHidden ? "text-[#bbb]" : "text-[#1a6e3e]"}`}
                            >
                              ₹{fmt(row.paid)}
                            </td>
                            <td
                  ...
                  <tfoot>
                    <tr className="bg-gradient-to-r from-[#1C3044] to-[#27435B]">
                      <td className="px-4 py-2.5 font-bold text-white text-[13px]">
                        TOTAL
                      </td>
                      <td className="px-4 py-2.5 font-bold text-white whitespace-nowrap">
                        ₹{fmt(grandTotal)}
                      </td>
                      <td className="px-4 py-2.5 font-bold text-[#7ddfb0] whitespace-nowrap">
                        ₹{fmt(grandPaid)}
                      </td>
                      <td
                        className={`px-4 py-2.5 font-bold whitespace-nowrap ${grandPending > 0 ? "text-[#f9a8a8]" : "text-[#7ddfb0]"}`}
                      >
                        ₹{fmt(grandPending)}
                      </td>
                      <td />
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-[#edf7f1] border border-[#b2dfc6] rounded-xl p-4 text-center">
              <div className="text-[10px] font-bold text-[#4A6B80] uppercase tracking-wider mb-1">
                Total Paid
              </div>
              <div className="text-lg font-bold text-[#1a6e3e]">
                ₹{fmt(grandPaid)}
              </div>
            </div>
            <div
            ...
            >
              ...
              <div
                className={`text-lg font-bold ${grandPending > 0 ? "text-[#a33030]" : "text-[#1a6e3e]"}`}
              >
                ₹{fmt(grandPending)}
              </div>
            </div>
