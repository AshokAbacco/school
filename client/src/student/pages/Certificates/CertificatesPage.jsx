  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ fontSize: 20, opacity: 0.22, lineHeight: 1 }}>
          {icon}
        </span>
      </div>
...
function CertCard({ cert, onView }) {
  const cat = CAT_STYLE[cert.category] ?? {
    bg: "#F3F4F6",
    color: "#6B7280",
    icon: "🏅",
  };
  const src = SRC_STYLE[cert.source] ?? {
    bg: "#F3F4F6",
    color: "#6B7280",
    label: cert.source,
    icon: "📄",
    ...
    >
      ...
      <div style={{ padding: "14px 16px" }}>
        ...
        >
          ...
          >
            {cat.icon}{" "}
            {cert.category?.charAt(0) + cert.category?.slice(1).toLowerCase()}
          </span>
          <span
            style={{
              fontSize: 11,
        ...
        {/* Event name */}
        {cert.eventName && (
          <p style={{ fontSize: 11, color: C.mid, margin: "0 0 5px" }}>
            📅 {cert.eventName}
          </p>
        )}
      ...
      >
        ...
        >
            ...
            >
              {printing ? (
                <>
                  <Loader2
                    size={15}
                    style={{ animation: "cert-spin 1s linear infinite" }}
                  />{" "}
                  Preparing…
                </>
              ) : (
          ...
          >
            ...
            <strong style={{ color: C.pale }}>Destination: Save as PDF</strong>{" "}
            → enable{" "}
            <strong style={{ color: C.pale }}>Background graphics</strong> →
            Save
          </p>
        </div>
      </div>
    ...
    <>
      ...
      >
        ...
        {!loading && !error && (
          ...
          >
            Showing {filtered.length} of {certs.length} certificate
            {certs.length !== 1 ? "s" : ""}
          </p>
        )}
