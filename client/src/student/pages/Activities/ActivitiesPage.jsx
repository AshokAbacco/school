...
const activityIcon = (name = "") => {
  ...
  if (n.includes("music") || n.includes("band") || n.includes("choir"))
    return Music;
  if (n.includes("basket") || n.includes("football") || n.includes("cricket"))
    return Dumbbell;
  if (n.includes("art") || n.includes("paint") || n.includes("draw"))
    return Palette;
  if (n.includes("photo")) return Camera;
  if (n.includes("environ") || n.includes("nature") || n.includes("eco"))
    return Globe;
  if (n.includes("debate") || n.includes("quiz") || n.includes("science"))
    return Brain;
  if (n.includes("drama") || n.includes("theater") || n.includes("dance"))
    return Music;
  return BookOpen;
};
...
function ActivityCard({ activity, onEnroll, onWithdraw, enrolling }) {
  ...
  const classes =
    activity.activityClasses?.map((ac) => ac.classSection.name) ?? [];
  const shownClasses = classes.slice(0, 3);
  const extraClasses = classes.length - 3;

  return (
    <div
      className="act-card-hover"
      style={{
        background: C.white,
        borderRadius: 12,
        border: `1px solid ${C.border}`,
      ...
      >
        ...
        <span
          style={{
            ...
            borderRadius: 20,
            background:
              activity.participationType === "TEAM"
                ? "rgba(59,130,246,0.10)"
                : "rgba(34,197,94,0.10)",
            color:
              activity.participationType === "TEAM" ? "#2563eb" : "#16a34a",
          }}
        >
        ...
        {shownClasses.map((cls) => (
          <span
            ...
            style={{
              ...
              padding: "2px 8px",
              borderRadius: 20,
              background: "rgba(189,221,252,0.40)",
              color: C.mid,
            }}
          >
            {cls}
        ...
        {isEnrolled ? (
          ...
          >
            {enrolling === activity.id ? (
              <Loader2
                size={10}
                style={{ animation: "act-spin 0.7s linear infinite" }}
              />
            ) : (
              <X size={10} />
            )}
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        >
          <p
            style={{ fontSize: 13, fontWeight: 600, color: C.dark, margin: 0 }}
          >
            {event.eventName}
          </p>
          <span
        ...
        {event.activity && (
          <p style={{ fontSize: 11, color: C.mid, margin: "0 0 4px" }}>
            {event.activity.name}
          </p>
        )}
        <div
    ...
    >
      ...
      {achievement.eventDate && (
        <p style={{ fontSize: 11, color: C.mid, flexShrink: 0 }}>
          {new Date(achievement.eventDate).toLocaleDateString("en-GB", {
            month: "short",
            year: "numeric",
          })}
        </p>
...
export default function ActivitiesPage() {
  ...
  const handleEnroll = async (id) => {
    setEnrolling(id);
    try {
      await apiFetch(`/${id}/enroll`, { method: "POST" });
      push("Enrolled successfully!");
      load();
    } catch (e) {
      push(e.message, "error");
    } finally {
  ...
  const handleWithdraw = async (id) => {
    setEnrolling(id);
    try {
      await apiFetch(`/${id}/enroll`, { method: "DELETE" });
      push("Withdrawn from activity");
      load();
    } catch (e) {
      push(e.message, "error");
    } finally {
  ...
  const statCards = [
    {
      label: "Activities",
      value: summary?.enrolledActivities ?? enrolledCount,
      sub: summary?.academicYear ?? null,
      accent: C.light,
      Icon: BookOpen,
    },
    {
      label: "Events Attended",
      value: summary?.eventsParticipated ?? events.length,
      sub: "total events",
      accent: "#a855f7",
      Icon: Trophy,
    },
    {
      label: "Achievements",
      value: summary?.achievements ?? achievements.length,
      sub: "awards earned",
      accent: "#f59e0b",
      Icon: Medal,
    },
            ...
            >
              ...
              <span
                style={{
                  ...
                  padding: "1px 7px",
                  borderRadius: 20,
                  background: "rgba(189,221,252,0.4)",
                  color: C.mid,
                  marginLeft: "auto",
                }}
              >
            ...
            <div style={{ padding: "8px 6px" }}>
              {events.length === 0 ? (
                ...
                <div className="act-2col" style={{ padding: "4px" }}>
                  {events.map((ev, i) => (
                    <EventRow key={`${ev.eventId}-${i}`} event={ev} />
                  ))}
                </div>
              )}
            </div>
